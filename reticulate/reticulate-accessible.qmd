...
Add `knitr::knit_engires$set(python = reticulate::eng_python)` to the setup chunk to set up the reiculate Python engine (not required for knitr \>= 1.18).
...
Requires reticulate plus RStudio v1.2+. Some features require v1.4+. Screenshot Here
...
-   `py_discover_config()`: Return all detected version of Python. Use `py_config()` to check which version has been loaded.
...
-   `virtualenv_list()`: List all available virtualenvs. Also `virtualenv_root()`.
...
-   `virtualenv_install(envname, packages, ...)`: Install a package within a virtualenv. Also `virtualenv_remove()`.
...
-   `conda_installs(envname, packages, ...)`: Install a package within a conda env. Also `conda_remove()`.
...
2.  The instances referenced by `use_` functions if called before `import()`. Will fail silently if called after impoty unless `required = TRUE`.
