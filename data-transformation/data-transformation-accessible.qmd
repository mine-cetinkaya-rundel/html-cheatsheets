...
-   `count(.data, ..., wt = NULL, sort = FLASE, name = NULL)`: Count number of rows in each group defined by the variables in `...`. Also `tally()`.
...
-   `across(.cols, .fun, ..., .name = NULL)`: Summarise or mutate multiple columns in the same way.
...
`mutate()` and`transmutate()` apply vectorized functions to columns to create new columns. Vectorized functions take vectors as input and return vectors of the same length as output.
